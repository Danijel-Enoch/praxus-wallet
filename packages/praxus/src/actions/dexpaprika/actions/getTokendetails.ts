...
export type GetTokenDetailsContent = z.infer<typeof GetTokenDetailsSchema> &
    Content;

export const isGetTokenDetailsContent = (
    obj: unknown
): obj is GetTokenDetailsContent => {
...
export const GET_TOKEN_DETAILS = {
    ...
    handler: async (
        ...
        try {
            ...
            elizaLogger.log(
                `Fetching details for token ${tokenAddress} on ${networkName}...`
            );
            ...
            const priceFormatted = token.price_usd
                ? `$${Number(token.price_usd).toLocaleString()}`
                : "N/A";
            ...
            elizaLogger.success(
                `Successfully retrieved token details for ${tokenAddress}!`
            );
            ...
            if (axios.isAxiosError(error)) {
                if (error.response?.status === 404) {
                    errorMessage =
                        "Token not found. Please check the network and token address and try again.";
