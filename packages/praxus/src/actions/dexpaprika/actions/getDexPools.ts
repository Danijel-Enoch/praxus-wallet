...
export const isGetDexPoolsContent = (
    obj: unknown
): obj is GetDexPoolsContent => {
...
export const GET_DEX_POOLS: Action = {
    name: "GET_DEX_POOLS",
    similes: [
        "DEX_LIQUIDITY_POOLS",
        "LIST_POOLS_BY_DEX",
    ...
    handler: async (
        ...
        try {
            ...
            elizaLogger.log(
                `Fetching top ${limit} pools on ${dexName} (${networkName}) ordered by ${orderBy} ${sort}...`
            );
            ...
            elizaLogger.success(
                `Successfully retrieved ${pools.length} pools on ${dexName} (${networkName})!`
            );
            ...
            if (axios.isAxiosError(error)) {
                if (error.response?.status === 404) {
                    errorMessage =
                        "Network or DEX not found. Please check the IDs and try again.";
